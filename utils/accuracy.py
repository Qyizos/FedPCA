...
def compute_metrics(pred, gt, names):
    ...
    if 'haus' in names:
        results['dice'], results['haus'] = accuracy_object_level(pred_labeled, gt_labeled, True)
...
def pixel_iou(gt, pred):
    mask = gt
    predict = pred
    mask_shape = mask.shape
    dice_map = np.zeros(mask_shape)
    dice_map[predict == 1] += 1
    dice_map[mask == 1] += 2
    tp = np.count_nonzero(dice_map == 3)
    pred_true_union = np.count_nonzero(dice_map)

    iou = float(tp) / pred_true_union
    return iou


if __name__=='__main__':

    aa = np.random.randint(low=0, high=100, size=(1000, 1000), dtype=np.int32)
    bb = np.random.randint(low=0, high=100, size=(1000, 1000), dtype=np.int32)
    out1 = pixel_iou(aa, bb)
    print(out1)
